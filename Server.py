...
from textual.widgets import Button, Label, LoadingIndicator, Static, Switch
...
class ChatServer(Screen):
    ...
    def __init__(self, *args, **kwargs):
        ...
        while not done:
            msg_recieved = client.recv(1204).decode("utf-8")
            raw_message = encoder.get_rawData(
                msg_recieved[0 : len(msg_recieved) - len(shared_key) + 1]
            )
            ...
            if not error == None:
                msg_recieved, error_corrected, err_idx = encoder.hamming_decode(
                    msg_recieved[0 : len(msg_recieved) - len(shared_key) + 1]
                )
                # self.text_label.update(f'Error corregido: {error_corrected} en el indice: {err_idx}')
                ...
                # self.text_label2.update(f'Mensaje recibido: {msg_recieved}')
            else:
    ...
    def compose(self) -> None:
        ...
        yield ScrollableContainer(
            Center(
                Label(
                    "CONECTADO CORRECTAMENTE CON EL CLIENTE, BIENVENIDO AL SERVER!!!",
                    id="msg_bienvenida",
                ),
